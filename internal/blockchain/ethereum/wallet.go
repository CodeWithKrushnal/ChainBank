...
func CreateWallet(password string) (string, *ecdsa.PrivateKey, error) {
...
func TransferFunds(fromPrivateKeyHex string, fromAddressHex string, toAddressHex string, amount *big.Int, gasPrice *big.Int, gasLimit uint64, chainID *big.Int) (*types.Transaction, error) {
	// Convert addresses
	...
	// Get the nonce
	nonce, err := EthereumClient.PendingNonceAt(context.Background(), fromAddress)
	if err != nil {
...
func PreloadTokens(walletAddress string, amount *big.Int) error {
	...
	signedTx, err := TransferFunds(fromPrivateKeyHex, fromAddressHex, toAddress, amount, gasPrice, gasLimit, chainID)
	if err != nil {
	...
	err = EthereumClient.SendTransaction(context.Background(), signedTx)
	if err != nil {
