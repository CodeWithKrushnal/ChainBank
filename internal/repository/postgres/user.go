...
func CreateUser(username, email, passwordHash, fullName, dob, walletAddress string, role int) error {
	_, err := DB.Exec(userRegisterQuery, username, email, passwordHash, fullName, dob)
	if err != nil {
	...
	_, err = DB.Exec(roleAssignmentQuery, user.ID, role)
	...
	_, err = DB.Exec(updateWalletIDQuery, walletAddress, user.ID)
	if err != nil {
...
func GetUserByEmail(email string) (User, error) {
	var user User
	err := DB.QueryRow(getUserByEmailQuery, email).Scan(&user.ID, &user.Username, &user.Email, &user.Password, &user.CreatedAt)
	return user, err
}
...
func UpdateLastLogin(userID string) error {
	...
	result, err := DB.Exec(updateLastLoginQuery, time.Now(), userID)
...
func UserExists(userName, email string) (usernameAlreadyInExistance, emailAlreadyInExistance bool, err error) {

	//Check if username already Exists
	err = DB.QueryRow(usernameAlreadyInExistanceQuery, userName).Scan(&usernameAlreadyInExistance)
	...
	err = DB.QueryRow(emailAlreadyInExistanceQuery, email).Scan(&emailAlreadyInExistance)
...
func GetUserHighestRole(userID string) (int, error) {

	var highestRoleLevel int

	// Query role assigned to the user.
	err := DB.QueryRow(getUserRolesQuery, userID).Scan(&highestRoleLevel)
	if err != nil {
		log.Printf("Error executing query: %v", err)
